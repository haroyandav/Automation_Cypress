...
function transformDriver(driver) {
    ...
    if (driver.proxyUrl || ((_a = driver.proxy) === null || _a === void 0 ? void 0 : _a.url)) {
        const agent = new proxy_agent_1.default({
            ...(0, url_1.parse)((_b = driver.proxyUrl) !== null && _b !== void 0 ? _b : driver.proxy.url),
            rejectUnauthorized: false,
        });
        const originalCallback = agent.callback.bind(agent);
        agent.callback = (request, options, callback) => {
...
async function getCapabilities(browser) {
    var _a;
    try {
        const caps = await ((_a = browser.getSession) === null || _a === void 0 ? void 0 : _a.call(browser));
        if (caps && utils.types.isObject(caps))
            return caps;
        return browser.capabilities;
    }
    catch (error) {
        if (/Cannot call non W3C standard command while in W3C mode/.test(error.message))
            return browser.capabilities;
